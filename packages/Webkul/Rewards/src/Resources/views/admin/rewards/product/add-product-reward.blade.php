<x-admin::layouts>
    <!-- Title -->
    <x-slot:title>
        @lang('rewards::app.admin.rewards.products.index.create.title')
    </x-slot:title>

    <v-add-product-reward></v-add-product-reward>

    @pushOnce('scripts')
        <script
            type="text/x-template"
            id="v-add-product-reward-template"
        >
            <x-admin::form action="{{ route('admin.reward.product.store_product_reward') }}">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex gap-x-2.5 items-center">
                        ...
                        <div class="flex gap-x-2.5 items-center">
                            ...
                            >
                                @lang('rewards::app.admin.rewards.products.index.create.save-btn')
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <script type="module">
            app.component('v-add-product-reward', {
                template: '#v-add-product-reward-template',
                data() {
                    return {
                        results: [],
                        productId: '',
                ...
                methods: {
                    searchProduct($event) {
                        if ($event.target.value.length > 2) {                            
                            this.$axios.post("{{ route('admin.reward.product.get-product') }}", { query: $event.target.value })
                                .then(response => {
